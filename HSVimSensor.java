//package org.hackystat.stdext.sensor.vim;
import java.util.*;
 ...
 *
 * @author Dan Port
 * @version $Id: HSVimSensor.java,v 1.11 2004/11/24 03:27:02 dport Exp $
 */
public class HSVimSensor {
	...
	private SensorProperties theProperties;

	private Properties mySensorProps = new Properties();
	...
	private void initSensor() {
		...
		// Start instance of SensorShell for this sensor
		}
		theProperties = new SensorProperties();
		...
		if(sensorshellHomePath == null) {
			outStream.println("Error: HACKYSTAT_SENSORSHELL_HOME property must be set in sensors.properties");
			theSensorShell.quit();
			System.exit(1);
		}
		...
		if(HSDataPath == null) HSDataPath = "~/.hackystat/vim/HS_VIM_DATA.dat";
		...
		String stalenessProp =
				theProperties.getProperty("HACKYSTAT_VIM_SENSOR_STALENESS_TOLERANCE");
		int STALENESS_TOLERANCE =
				(stalenessProp != null)? Integer.parseInt(stalenessProp) : 5;
	...
	/**
	 * Updates VIM data from Vim data file. It sets the current vim file name
	 * and returns the number of bytes in the vim buffer for that file
	 *
	...
	private long readCurrentVimDataFile() {
		...
		if (vimDataFile != null && vimDataFile.exists()) {
			try {
				...
				/*
				 * // parse the line of data into space separted tokens String[]
				 * result = vimData.split(" "); for (int x = 0; x <
				 * result.length; x++) { outStream.println(result[x]); }
				 */
				String[] tmp;
				...
				/**
				 * check timestamp for staleness in case of vim crash or
				 * irregular exit that left bogus HS Vim data file
				 */
...
//			sent:
	...
	/**
	 * Checks that the timestamp is resonable (within some multiple of sample
	 * time) and removes the HS Vim datafile and exits if not.
	 *
	 * <TBD> timestamp staleness check not implemented yet! Does nothing now...
	 * Maybe this should just compare the timestamp with the file mod time?
	 *
	 * @param timestamp
	 *            Time stamp.
	 */
	...
	/**
	 * Takes a String path value and replaces the ~ with the users
	 * home directory (as set in the HOME system env variable). If no ~\
	 * is at the head, then the original path value is returned unchanged.
	 *
	 * @param path
	 *            Path string.
	 */
